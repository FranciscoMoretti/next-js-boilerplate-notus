          ...
          <div className="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
            <div className="relative grid gap-6 bg-white dark:bg-gray-800 px-5 py-6 sm:gap-8 sm:p-8 divide-y divide-gray-100">
              {props.items.map((item) => {
                return (
                  <a href={item.link || '#'} key={item.label} className="-m-3 p-3 flex items-center hover:bg-gray-50">
                    {item.icon}

                    <div className="ml-4">
                      <p className="text-base font-medium text-gray-900 dark:text-white">{item.label}</p>
                      {item.desc && <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{item.desc}</p>}
                    </div>
                  </a>
                );
              })}
